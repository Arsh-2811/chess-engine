...
void Utils::getCastling(std::string const& castleString, unsigned int &castlingRights){
	...
	if (castleString[0] == '-'){
		...
		for (unsigned int i=0; i<castleString.size(); ++i){
			if (castleString[i]=='K') castlingRights |= 0x1;
			else if (castleString[i]=='Q') castlingRights |= 0x2;
			else if (castleString[i]=='k') castlingRights |= 0x4;
			else if (castleString[i]=='q') castlingRights |= 0x8;
		}
	}
}
...
string Utils::Move16ToShortString(const Move16 shortMove){
